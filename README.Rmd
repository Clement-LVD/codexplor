...
> `codexplor` help me to figure out the big picture of a programming project faster, and to manage it more efficiently. I've used it, for example, to pinpoint where to start a polishing loop, to identify all the functions impacted by upcoming changes, or to assess the impact of the dev loop on the project.
...
Will return an interactive dataviz' of the internal dependancies within `codexplor` :
...
names(fn_segmentation) <- c("Analyze a programming project (High-level functions)" , "Low-level & helper functions" )
