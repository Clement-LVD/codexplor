...
⏩ Assess local complexity with **Text-mining metrics** *[planned feature]* 
...
1. Give some folders path(s) and/or urls, compute a Citations Network of the functions with   ![.](https://img.shields.io/badge/{Scraping}-bold?style=flat&logoColor=black&logoSize=2&label=get_text_network_from_project()&labelColor=yellow&color=black)
...
2. Turn it into a directed `igraph`  network with ![1.](https://img.shields.io/badge/{igraph}-bold?style=flat&logoColor=black&logoSize=2&label=get_igraph_from_df()&labelColor=green&color=black)
...
The func' used by others func' are colorized, such as their ingoing links. For ex. we quicky see the most-common local dependancy (i.e. `compat-types-check.R` for the tidyverse/stringr repo).
