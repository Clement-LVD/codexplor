...
test_that("get_regex_from_vector fonctionne avec des préfixes et suffixes simples", {
  ...
  result <- get_regex_from_vector(words, "^", "$", n_char_to_add_suffix =10)
...
test_that("get_regex_from_vector échappe les caractères spéciaux", {
  words <- c("foo(bar)", "baz(qux)", 'a')
  result <- get_regex_from_vector(words, prefix_for_regex = "^",suffix_for_regex =  "$", n_char_to_add_suffix =10)
  # Résultat attendu : "^foo\\(bar\\)$|^baz\\(qux\\)$"
  expected <- "^foo\\(bar\\)|^baz\\(qux\\)|^a$"
  expect_equal(result, expected)
})
...
test_that("get_regex_from_vector work with only one element", {
  ...
  # Résultat attendu : "^apple" (default don't add suffix)
  expected <- "^apple"
  expect_equal(result, expected)
})
...
test_that("get_regex_from_vector fonctionne avec des préfixes et suffixes personnalisés", {
  ...
  result <- get_regex_from_vector(words, prefix_for_regex = "(", suffix_for_regex = ")", , n_char_to_add_suffix =4)
