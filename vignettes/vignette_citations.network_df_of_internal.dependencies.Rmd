...
- The 1st matches of `codexplor::construct_corpus` will extract exposed functions names from files where these functions are *defined*, e.g., have found 'Pier', 'pol' and 'jak' function names in 3 separate files.
- The 2nd matches show the files or the functions that *calls* these functions, e.g., a programming file is citing 'Pier(', 'pol(' and 'jak(' and thus this file is linked to the 3 files where these functions are defined or to these precise functions, depending on the desired level-of-depth. 
