...
The `codes` `dataframe` (one of the `corpus.lines` dataframes) have several additionnal columns, such as :

- a `commented` column with comments at the end of a codeline
- and a '`matches`' column that contain functions names (see hereafter). 
...
, "e.g., extract unexposed content accordingly to the block of code delimitor such as '{' and '}' for the R language"
...
rownames(steps) <- c("1. Read", "2. Preprocess" ,"3. Identify", "3. Qualify")
...
`codexplor::get_doc_network_from_project` is an helper 'high-level' function that perform an analysis of the internal dependancies by performing a 2nd matches from the 1st matches of `construct_corpus`, e.g., see the [vignette about analyzing a network of internal dependancies with `get_doc_network_from_project`](https://clement-lvd.github.io/codexplor/articles/vignette_analyse_citations_network_from_project.html).
