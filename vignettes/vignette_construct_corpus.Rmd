...
# 1) Construct a corpus without a Citations network
  corp <- construct_corpus(languages = "R", repos =  "clement-LVD/codexplor")
...
Further complementary analyzes will add others dataframes to the list, e.g., a network of internal dependencies (classes `citations.network`, `internal.dependencies` & `data.frame`). This `corpus.list` of df offers insights from system level to the module level (complete `citations.network` > `files` > `functions`).
...
, Level = c("Line of code" , "Commented line","Function-level metrics"
 ...
 , e.g. = c( "Identify problematics lines, e.g., longest ones", '.', "e.g., quantify the longest func' and those with several internal dependencies", "e.g., quantify number of functions within files and critical internal dependencies", "Doc-to-doc (add metrics to the `files` level) or functions network (add metrics to the `functions` level)" )
))
