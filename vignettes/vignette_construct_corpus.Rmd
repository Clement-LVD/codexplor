...
**1st matches operated.** Given language(s), `construct_corpus` try to perform an initial identification within the content. By default, these 1st matches are supposed to be the function names, as soon as a function is defined within a file. For example, if a `pol  <- function` is defined within a file, the `matches` corresponding entry will be `'pol'`.

`construct_corpus` rely on some default patterns, associated with the supported languages. 
