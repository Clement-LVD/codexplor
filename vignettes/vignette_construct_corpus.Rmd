...
Complementary analyzes will add others dataframes to the list, e.g., a network of internal dependencies (classes `citations.network` & `dataframe`).
...
- a `commented` column with inlines comments at the end of a codeline
...
`codexplor::get_doc_network_from_project` is an helper 'high-level' function that perform an analysis of the internal dependencies by performing a 2nd matches from the 1st matches of `construct_corpus`, e.g., see the [vignette about analyzing a network of internal dependencies with `get_doc_network_from_project`](https://clement-lvd.github.io/codexplor/articles/vignette_analyse_citations_network_from_project.html).
