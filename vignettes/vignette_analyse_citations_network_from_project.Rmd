...
The 1st matches of `codexplor::construct_corpus` let us find some files that *defines* a function, and the 2nd matches of `compute_doc_network_from_corpus` are the files that used these functions. Thus, links are beginning from the programming file that is calling a function => to the programming file where this function is defined. See [vignette of `construct_corpus`](https://clement-lvd.github.io/codexplor/articles/vignette_construct_corpus.html) for the 1st matches; and the [vignette of the `citations.network` `dataframe`](https://clement-lvd.github.io/codexplor/articles/vignette_citations.network_df_of_internal.dependencies.html) for the 2nd matches.
...
```{r construct-corpus-and-network, echo=TRUE}
require(codexplor)
