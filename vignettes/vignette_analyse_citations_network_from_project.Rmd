...
For example, the most-common local dependency inside the tidyverse/stringr repo are `modifiers.R` and `compat-types-check.R`. Next, there is other text-mining metrics that will give us insights about these files, e.g., one should verify that the local complexity of this `compat-types-check.R` file is reported as a 'trivial' level (see next section).
