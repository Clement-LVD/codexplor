...
This vignette give a basic understanding of the `list` of the `citations.network` dataframe returned by `codexplor::get_doc_network_from_project`.
...
2. and add a new `data.frame` with additional classes `citations.network` and `internal.dependencies`, as the edgelist of a network of the functions defined within the files of the corpus (see the [vignette of the `citations.network` `dataframe`](https://clement-lvd.github.io/codexplor/articles/vignette_citations.network_df_of_internal.dependencies.html)). 
